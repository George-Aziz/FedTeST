...
class ViT(nn.Module):
    ...
    def trainable_parameters(self):
        return [para for para in self.parameters() if para.requires_grad]

    def adapted_parameters(self, target='all'):
        if target == 'all':
            return [p for p in self.parameters() if p.requires_grad]
        elif target == 'attn_out':
            params = []
            for attn_pre_norm, _ in self.transformer.layers:
                to_out_linear = attn_pre_norm.fn.to_out[0]
                params += [to_out_linear.weight, to_out_linear.bias]
            return params
        else:
