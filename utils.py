...
def compute_accuracy(model, dataloader, criterion, get_confusion_matrix=False, device="cpu"):
    ...
    with torch.no_grad():
        for tmp in dataloader:
            for batch_idx, (x, target) in enumerate(tmp):
                ...
                total_loss += loss.item() # sum loss weighted by batch size
...
# Computes accuracy with adaptation rates
def compute_accuracy_ttp(model, dataloader, adapt_lrs, spv_criterion, unspv_criterion, ttp_method, get_confusion_matrix=False, device="cpu", adapt_params=None):
    ...
    for tmp in dataloader:
        for batch_idx, (x, target) in enumerate(tmp):
            if ttp_method == "online_avg": # Evaluation with online average method
                avg_state = copy.deepcopy(model.state_dict())  # The average state of the model at the current time step (initial iteration would not be average yet)
                model.load_state_dict(model_state)
            elif ttp_method == "batch": # Evaluation per batch method
                model.load_state_dict(model_state)

            x, target = x.to(device), target.to(device,dtype=torch.int64)
            
            # === UNSUPERVISED ADAPTATION ===
            unspv_grad = unspv_adapt(model, current_lrs, x, target, unspv_criterion, 'Image', adapt_params=adapt_params)

            # === SUPERVISED EVALUATION ===
            if ttp_method == 'online_avg':
...
def unspv_adapt(net, adapt_lrs, x, y, unspv_criterion, task='Image', adapt_params=None):
    if adapt_params is None:
        adapt_params = net.trainable_parameters()

    net.eval() # Consistent with unsupervised adaptation during testing/evaluation

    if task == 'Image':
        out = net(x)
        loss = unspv_criterion(out, y)

    elif task == 'NLP':
        y_pred, _ = net(x)
        loss_vec = unspv_criterion(y_pred, y)  # unsupervised loss per sample
        loss = loss_vec.mean()

    loss.backward()

    unspv_grad = [p.grad.clone() for p in adapt_params]
    with torch.no_grad():
        for i, (p, g) in enumerate(zip(adapt_params, unspv_grad)):
            p -= adapt_lrs[i] * g

    net.zero_grad()
    return unspv_grad

def weighted_avg_state(state1, state2, lamda):
    return {k: lamda * state1[k] + (1 - lamda) * state2[k] for k in state1}

# Computes accuracy with adaptation rates
def compute_accuracy_shakes_ttp(model, dataloader, adapt_lrs, unspv_criterion, ttp_method, device="cpu", adapt_params=None, args=None):
    ...
    # Compute adapt_params if not provided
    if adapt_params is None:
        adapt_target = getattr(args, 'adapt_target', 'all') if args else 'all'
        adapt_params = (model.adapted_parameters(adapt_target)
                        if hasattr(model, 'adapted_parameters') else model.trainable_parameters())
    ...
    for tmp in dataloader:
        for batch_idx, (x, y, indices) in enumerate(tmp):
            ...
            # === UNSUPERVISED ADAPTATION ===
            unspv_grad = unspv_adapt(model, current_lrs, x, y, unspv_criterion, 'NLP', adapt_params=adapt_params)
            ...
            with torch.no_grad():
                y_pred, _ = model(x)
                global_loss += spv_criterion(y_pred, y).sum().item() / chunk_len
...
# Computes accuracy with adaptation rates
# By default we do progressive adaptation as we do training but there are two alternatives: batch and online_avg
def compute_accuracy_loss_ttp(model, dataloader, adapt_lrs, ttp_method, device="cpu", adapt_params=None):
...
def compute_accuracy_per_client_simple(global_model, args, net_dataidx_map_train, net_dataidx_map_test, adapt_lrs=None, nets=None, device="cpu"):
    if args.train_acc_pre:
        train_results = defaultdict(lambda: defaultdict(list))
    test_results = defaultdict(lambda: defaultdict(list))
    for net_id in range(args.n_parties):
...
def compute_accuracy_per_client_ttp(hyper, nets, global_model, args, net_dataidx_map_train, net_dataidx_map_test, num_class, adapt_lrs, ttp_method, device="cpu"):  
    hyper.eval()
    if args.train_acc_pre:
        train_results = defaultdict(lambda: defaultdict(list))
    test_results = defaultdict(lambda: defaultdict(list))
    ...
    for net_id in range(args.n_parties):
        ...
        if args.dataset == "shakespeare":
            ...
            _adapt_params = (local_model.adapted_parameters(getattr(args, 'adapt_target', 'all'))
                                 if hasattr(local_model, 'adapted_parameters') else None)
            test_correct, test_total, test_avg_loss = compute_accuracy_shakes_ttp(local_model, test_dl_local, adapt_lrs, unspv_criterion, ttp_method, device=device, adapt_params=_adapt_params, args=args)
            if args.train_acc_pre:
                train_correct, train_total, train_avg_loss = compute_accuracy_shakes_ttp(local_model, train_dl_local, adapt_lrs, unspv_criterion, ttp_method, device=device, adapt_params=_adapt_params, args=args)
            ...
            if args.k_neighbor:
                ...
                _adapt_params = (local_model.adapted_parameters(getattr(args, 'adapt_target', 'all'))
                                 if hasattr(local_model, 'adapted_parameters') else None)
                test_correct, test_total, test_avg_loss = compute_accuracy_loss_ttp(local_model, test_dl_local, adapt_lrs, ttp_method, device=device, adapt_params=_adapt_params)
                if args.train_acc_pre:
                    train_correct, train_total, train_avg_loss = compute_accuracy_loss_ttp(local_model, train_dl_local, adapt_lrs, ttp_method, device=device, adapt_params=_adapt_params)
